...
function SpawnVegetation(x, y, z){
	...
	let index = PerlinNoise(x/Settings.BlockWidth, z/Settings.BlockWidth, 0, 5)
...
function SpawnTree(x, y, z){
	...
	let leafLayers = Settings.LeafArrangement
...
function ReplaceChunkZ(dir){
	...
	Scene.meshes.filter(mesh=>mesh.position.z==farz).forEach(mesh=>{
...
function ReplaceChunkX(dir){
	...
	Scene.meshes.filter(mesh=>mesh.position.x==farx).forEach(mesh=>{
		mesh.setEnabled(false)
		Scene.removeMesh(mesh)
	})
